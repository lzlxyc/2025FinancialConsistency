...
model_name = '/DATA/disk0/public_model_weights/qwen/Qwen2.5-7B-Instruct'
...
def exteract_module_raw_text(module_name, lines):
    messages=[
        {"role": "system", "content": f"""你是一个专业的保险行业的信息处理专家，请对下面的文本中抽取出与{module_name}相关的文本，只需要输出原文，不要有其他输出。
        如果相关的本文本不存在，则输出一个空格。"""},
...
def do_inference(input_path="../DATA/测试 A 集/", save_file="submit.jsonl", nrows=None):
    c = 0
    df = pd.read_json(input_path+'data.jsonl', lines=True)
    ...
    for row in df.iterrows():
        ...
        try:
            module_content_list = []
            for path in glob.glob(f"{input_path}materials/{row[1].material_id}/*/*"):
                print(path)
            ...
            pred = bool(np.mean(result) < 0.3)
            with open(save_file, "a") as up:
                up.write(json.dumps({
    ...
    df['ypred'] = y_preds
    df.to_csv(save_file.replace('.jsonl','.csv'),index=0)
...
def do_eval():
    ...
    st = time()
    df = do_inference(input_path="../DATA/验证集/", save_file="../DATA/验证集/submit.jsonl")
    print(df.head())

    usetime = round(time() - st, 4)
    F1 = f1_score(df['result'], df['ypred'])
    print(f"******F1—score:{F1}   cost\s:{usetime}")

if __name__ == "__main__":
    do_inference(input_path="../DATA/测试 A 集/", save_file="../DATA/测试 A 集/submit.jsonl", nrows=10)
